...
export function SurveillanceQueueWorkspace() {
    ...
    <div className="space-y-6">
      ...
      {alerts.filter((a) => a.status === "confirmed").length > 0 && (
        ...
        >
          ...
          — immediate public health action is required. Review the Outbreak Alerts below.
        </AlertBanner>
      )}
              ...
              <div className="space-y-2">
                {reports.map((report) => (
                  <button
                    ...
                    key={report.id}
                    type="button"
                    onClick={() => router.push(`/surveillance/${report.id}`)}
                  >
