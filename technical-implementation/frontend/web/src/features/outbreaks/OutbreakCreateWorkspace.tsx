...
import type { HealthFacility, Patient, Vaccine, VaccineBatch } from "@/features/registry/types";
import type {
...
import { listFacilities, listPatients, listVaccineBatches, listVaccines } from "@/services/patients";
import { createSurveillanceReport } from "@/services/outbreaks";
import {
  InlineError,
  SelectInput,
  StatusPill,
...
export function OutbreakCreateWorkspace() {
  ...
  useEffect(() => {
    ...
    async function loadFormContext() {
      ...
      try {
        const [facilityRows, patientRows, vaccineRows, batchRows] = await Promise.all([
  ...
  async function handleCreateReport(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/outbreaks/${report.id}`);
    } catch (caughtError) {
  ...
  if (!canManageReports) {
    return (
      <div className="p-6">
        <InlineError message="You do not have permission to create case reports." />
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <form className="mt-6 grid gap-6" onSubmit={handleCreateReport}>
          ...
          {reportForm.surveillance_category === "aefi" ? (
            <section className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03]">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <SelectInput
                  label="Batch / lot"
                  value={reportForm.aefi_vaccine_batch}
                  onChange={(value) =>
                    setReportForm((current) => ({
                      ...current,
                      aefi_vaccine_batch: value,
                    }))
                  }
                  options={[
                    { label: "Not recorded yet", value: "" },
                ...
                <TextInput
                  label="Dose label"
                  value={reportForm.vaccine_dose_label}
                  onChange={(value) =>
                    setReportForm((current) => ({
                      ...current,
                      vaccine_dose_label: value,
                    }))
                  }
                />
                <TextInput
                  ...
                  type="date"
                  value={reportForm.vaccination_date}
                  onChange={(value) =>
                    setReportForm((current) => ({
                      ...current,
          ...
          <section className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
              Lab result and outcome
            </h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <SelectInput
                label="Specimen status"
                value={reportForm.specimen_status}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    specimen_status: value,
                  }))
                }
                options={[
              ...
              <TextInput
                label="Specimen type"
                value={reportForm.specimen_type}
                onChange={(value) =>
                  setReportForm((current) => ({ ...current, specimen_type: value }))
                }
              />
              <TextInput
                label="Collection date"
                type="date"
                value={reportForm.specimen_collection_date}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    specimen_collection_date: value,
                  }))
                }
              />
              <SelectInput
                label="Lab result"
                value={reportForm.lab_result_status}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    lab_result_status: value,
                  }))
                }
                options={[
              ...
              <TextInput
                label="Lab test type"
                value={reportForm.lab_test_type}
                onChange={(value) =>
                  setReportForm((current) => ({ ...current, lab_test_type: value }))
                }
              />
              <TextInput
                label="Result date"
                type="date"
                value={reportForm.lab_result_date}
                onChange={(value) =>
                  setReportForm((current) => ({ ...current, lab_result_date: value }))
                }
              />
              <SelectInput
                label="Outcome"
                value={reportForm.clinical_outcome}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    clinical_outcome: value,
                  }))
                }
                options={[
              ...
              <TextInput
                label="Outcome date"
                type="date"
                value={reportForm.clinical_outcome_date}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    clinical_outcome_date: value,
                  }))
                }
              />
              <TextInput
                ...
                type="date"
                value={reportForm.next_follow_up_date}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <TextAreaInput
                ...
                value={reportForm.lab_result_notes}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    lab_result_notes: value,
                  }))
                }
              />
              <TextAreaInput
                label="Outcome notes"
                value={reportForm.outcome_notes}
                onChange={(value) =>
                  setReportForm((current) => ({
                    ...current,
                    outcome_notes: value,
          ...
          <div className="flex justify-end border-t border-gray-200 pt-5 dark:border-gray-800">
            <button
              ...
              type="submit"
            >
              {isCreating ? "Submitting..." : "Submit case report"}
            </button>
          </div>
        </form>
      </section>
...
function normalizeReportPayload(
  ...
  return {
    ...
    aefi_vaccine: payload.surveillance_category === "aefi" ? payload.aefi_vaccine || null : null,
    aefi_vaccine_batch:
      payload.surveillance_category === "aefi" ? payload.aefi_vaccine_batch || null : null,
    vaccine_dose_label:
      payload.surveillance_category === "aefi"
        ? payload.vaccine_dose_label.trim() || null
        : null,
    vaccination_date:
      payload.surveillance_category === "aefi" ? payload.vaccination_date || null : null,
    lab_sample_taken: payload.specimen_status !== "not_collected",
    specimen_status: payload.specimen_status as CreateSurveillanceReportPayload["specimen_status"],
    specimen_type: payload.specimen_type.trim() || null,
    specimen_collection_date: payload.specimen_collection_date || null,
    lab_test_type: payload.lab_test_type.trim() || null,
    lab_result_status:
      (payload.lab_result_status as CreateSurveillanceReportPayload["lab_result_status"]) || null,
    lab_result_date: payload.lab_result_date || null,
    lab_result_notes: payload.lab_result_notes.trim() || null,
    clinical_outcome:
      (payload.clinical_outcome as CreateSurveillanceReportPayload["clinical_outcome"]) || null,
    clinical_outcome_date: payload.clinical_outcome_date || null,
    outcome_notes: payload.outcome_notes.trim() || null,
